...
| --- | --- | --- |
| `settings.input.bucket_dir` | `config.toml` `[input]` | 對到不存在的 `zone-mapping/bucket_name1`，找不到 registry |
| `OUTPUT_ROOT = outputs/` | `pipeline.py` 常數 | 去 `zone-mapping/outputs/` 找輸入而 `FileNotFoundError`，產出也落在錯的樹 |
...
```bash
uv run --directory zone-mapping ruff check .   # lint（line-length = 88，select = ["E", "F", "I", "W"]）
