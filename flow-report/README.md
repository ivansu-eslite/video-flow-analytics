...
| `models/registry.py` | `camera_registry.yaml` 的 Pydantic 模型與 zone 驗證 |
| `services/report.py` | 報表 orchestration、I/O 與 Excel 讀寫 |
| `services/stats.py` | 時區轉換、期間彙總、尖峰計算等純函式 |
| `observability/structured_logging.py` | 輸出單行 JSON 的 `StructuredLogger` |
...
| --- | --- | --- |
| `OUTPUT_ROOT = outputs/` | `config/constants.py` 常數 | 去 `flow-report/outputs/` 找輸入而 `FileNotFoundError`，報表也落在錯的樹 |
...
`[report]` 三個區塊，透過 pydantic-settings 載入；載入失敗時會印出警告並回退到各項
預設值。各欄位亦可用環境變數覆寫（巢狀分隔符 `__`，例如
`REPORT__PERIOD_MINUTES=30`）。範例：
...
```bash
uv run --directory flow-report ruff check .   # lint（line-length = 100，select = ["E", "F", "I", "W"]）
