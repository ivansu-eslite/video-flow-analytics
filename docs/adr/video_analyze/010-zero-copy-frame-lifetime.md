...
- **（issue #108／#116 後更新）上面兩條的容量數字已不成立**：影格縮放移到讀取端之後，
  ...
  **（2026-09-02 實測修正）「效能崩掉」的量級被兩組獨立實測推翻**：M15 `shm64` 格對照
  正常配置，`Mpx/s` 755.8 對 747.8／749.3，降級那格反而略高，差異在重跑雜訊內；M15 另外
  用 `/proc/maps` 獨立驗到同樣的 5 塊 `/dev/shm` ＋ 4 塊 `/tmp` 降級模式，掉到 `/tmp` 的
  那 45 MB 全程待在 page cache、沒碰過磁碟；C25 容器驗收在真正的容器
  （`python:3.12-slim --shm-size=64m`）裡另外實跑 9 次 `create_ring_buffer`，同樣 5 塊 +
  4 塊，全部成功不報錯，確認容器環境下這條降級路徑本身不出錯（出處：
  `outputs/vfa_perf/research/cloud-gap-20260902/SUMMARY-P40.md`）。**前提是記憶體充裕、
  `/tmp` 落在有 page cache 的機器**：記憶體吃緊或 `/tmp` 是慢速掛載時仍會付出代價；這輪
  驗到的是記憶體充裕的條件——Vertex 容器的實際上限已於同日查明（`memory.max` 26.1 GiB、
  `shm_total_mb` 25.7 GiB，遠高於上面的 202.5–405 MiB 需求），屬記憶體充裕情境，可以
  外推；但其他部署場景記憶體吃緊時仍要留意，不能假設放諸四海皆準。上面「配置前的擋」
  正當性不建立在這個量級上——靜默改變 backing store 本身就要 fail loud，
  `require_shm_capacity` 不變。
