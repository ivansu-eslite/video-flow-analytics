...
3. **T 的有效上界是漏偵測空洞的上界，超過之後 T 不再是在補漏偵測。** ByteTrack 的
   ...
   | T | 1.5 s | 2.0 s | 2.2 s | 3.0 s | 5.0 s | 10.0 s |
   |---|---:|---:|---:|---:|---:|---:|
   | 停留段數 | 2384 | 2232 | 2185 | 2071 | 1944 | 1844 |
   | `dwell_events` 總計 | 133 | 137 | 140 | 140 | 149 | 161 |

   T = 2.2 與 T = 3.0 的事件數都是 140，但**不能據此說「漏偵測的空洞在 2.2 秒就吃完了」**：
   這中間有 114 個停留段被併掉（2185 → 2071 段），0.1 秒一格細掃看得到事件數在 2.0–4.0
   之間震盪於 137–145（T = 2.4 時是 143）。**事件數對 T 不是單調的**——T 變大時，同一人
   原本各自達標的兩段被併成一段（−1），同時原本不達標的碎片被接起來達標（+1），兩個方向
   同時發生。單調遞減的是段落數（T 從 0.5 到 30 秒：3439 → 1763）。

   **採用預設 T = 3.0 秒**的理由因此是「落在平坦區」，不是「停在增長開始之前」：T 從 1.5
   拉到 5.0 秒（差 3.3 倍）事件數只從 133 動到 149（+12%），而 3.0 秒同時涵蓋各路 fps 差異
   （15 fps 的 30 格 = 2.0 秒，加上餘裕）。
...
- **相鄰 zone 共用一條邊時，站在邊上的同一個人被兩個 zone 各算一次**，兩邊的人次在時間上
  重疊。上一條講的是單一 zone 被高估，這一條是跨 zone 的重複，兩者不同。實測（PR #165 合併
  前的異常掃描，同樣是十個 zone 一小時 140 人次那批）：`平擺桌2_內側`／`平擺桌2_外側` 是同
  一張桌的兩側、共用一條邊（兩個多邊形沒有重疊面積，該顧客 15,767 列裡同時落在兩區的是 0
  列），一位站著翻書 12 分鐘的顧客落腳點逐格在邊的兩側跳，而 `dwell_events` 吃的是生的
  `in_zone`（Decision 1 刻意不吃 `entries` 那套線段區域的黏著狀態），零星幾格被 T 串成長段，
  於是兩個 zone 各累積出 5 段達標——**一個人貢獻 10 人次，佔那天 140 筆的 7.1%**。五組內／
  外側 zone 只有這一組出現時間重疊，其餘四組 0 對（1/108 人）。`flow_report` 依 zone 名稱
  分組，這個人在報表上會出現兩次；把同一張桌的內外側相加會再放大一次。**現況是接受、不改
  判定**（例如「同一人同一時刻只歸給一個 zone」）：改判定要引入跨 zone 的歸屬規則，而現象
  目前稀有；重新評估的訊號是內／外側這類共用邊的 zone 變多，或報表上同一張桌的兩側人次
  同時偏高。
