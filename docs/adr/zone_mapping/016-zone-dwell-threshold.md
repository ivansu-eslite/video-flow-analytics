...
10 個 zone；`test_cam002`／`003`／`009` 為 3840 寬、`test_cam007` 為 1920 寬）。重現腳本
`board/C26/zone_dwell_band.py`（開發用、不進版控，比照 `overlay/analysis/` 的慣例；
主工作樹唯讀，故未放在該目錄下）。
...
| 口徑 | 樣本數 | p50 | p99 | max |
|---|---:|---:|---:|---:|
| **所有列**（純漏偵測的空洞） | 2,129,692 | 67 ms | 333 ms | **2.133 s** |
| **只看區內列**（漏偵測＋真的踏出區域） | 199,472 | 50 ms | 450 ms | 417.9 s |

各攝影機的取樣間隔中位數為 50.0 ms（20 fps，三台 4K）與 66.67 ms（15 fps，`test_cam007`）。
...
- **T 會把真實的離區時間一起算進停留**：`elapsed` 是 `last − first`，含所有被橋接的
  空洞。實測區內列的間隔有 0.083% 落在 2–3 秒（在 T = 3.0 之內、卻超過漏偵測的 2.133 秒
  上界），共 114 個中斷是被橋接起來的真實離區。T 小時可忽略，T 大時就是實質灌水——這是
  「單一容忍參數」這個拍板的必然代價。
...
- `board/C26/zone_dwell_band.py`：判定基礎 A vs C 的比較腳本與本文數字的出處
  （開發用工具，不進版控）
