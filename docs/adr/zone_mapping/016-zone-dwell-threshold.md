...
- **相鄰 zone 共用一條邊時，站在邊上的同一個人被兩個 zone 各算一次**，兩邊的人次在時間上
  ...
  前的異常掃描，同樣是那份 15 分鐘素材、140 人次那批）：`平擺桌2_內側`／`平擺桌2_外側` 是同
  一張桌的兩側、邊界幾乎貼在一起（兩側相距約 2 px，兩個多邊形沒有重疊面積，該顧客 15,767
  列裡同時落在兩區的是 0 列），一位站著翻書 12 分鐘的顧客落腳點逐格在邊的兩側跳，而 `dwell_events` 吃的是生的
  `in_zone`（Decision 1 刻意不吃 `entries` 那套線段區域的黏著狀態），零星幾格被 T 串成長段，
  於是兩個 zone 各累積出 5 段達標——**一個人貢獻 10 人次，佔這批 140 筆的 7.1%**。五組內／
  外側 zone 只有這一組出現時間重疊，其餘四組 0 對。`flow_report` 依 zone 名稱
  分組，這個人在報表上會出現兩次；把同一張桌的內外側相加會再放大一次。**現況是接受、不改
  判定**（例如「同一人同一時刻只歸給一個 zone」）：改判定要引入跨 zone 的歸屬規則，而現象
  目前稀有；重新評估的訊號是內／外側這類邊界相貼的 zone 變多——報表上同一張桌兩側人次
  同時偏高不算訊號，那既可能是同一個人被算兩次、也可能兩側真的都很多人，要分辨得回到逐
  `track_id` 的時間重疊檢查。
...
- **母體小是預期值，不是 bug。** 規劃期實測（`bucket_20260728_small`，八路 3001 條軌跡）
  ...
  20 秒的只會更少。本次素材是 15 分鐘（14:00–14:15）、九路，十個 zone 共 140 人次，每個
  zone 是 9–26；`bucket_minutes = 60`，所以這 140 筆全部落在同一個 `time_bucket`。
...
- 判定基礎 A vs C 的比較腳本（開發期的一次性工具，不進版控）：Options Considered 那張
  A／C 對照表、逐 zone 的差異與「只有 A 算到的 5 段深入 27–42 px」出自它的輸出。
  **其餘數字不在該腳本裡**：Context 的間隔
  ...
  `zone_mapping` 後讀 `zone_counts.parquet` 得到的；**T 掃描表的「停留段數」一列、
  T = 1.5／2.0／2.2 三欄、0.1 秒一格的細掃與 3439 → 1763，連同 Negative 共用邊那條的數字，
  出自 PR #165 合併前的另一次異常掃描**（同一份輸入；兩次在 T = 3.0／5.0／10.0 三個重疊點
  上的事件數一致）；「既有兩欄逐列不變」是把 70bb3ab 的
