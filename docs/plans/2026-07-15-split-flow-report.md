...
- [ ] 資料輸入/輸出或 API 規格定義清楚：進入點 `export_report_daily(date, bucket_dir,
      ...
      output_root=OUTPUT_ROOT) -> Path` 簽章不變（**`output_root` 不可漏**——
      `tests/report/test_pipeline.py` 正是靠它注入 tmp 路徑，漏掉測試會壞）；讀
...
- **`metric='unique_visitors'` 為近似值**：既有已知限制（跨 bucket 停留會被重複計入），
  本次不修，僅原樣搬移。
- **`output_root` 參數漏掉**：`tests/report/test_pipeline.py` 靠它注入 tmp 路徑，漏掉會
  讓既有測試壞掉。
