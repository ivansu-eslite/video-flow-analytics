...
- 新增 analyze／zone-map 的測試（目前僅 report 有測試，隨 flow-report 一起搬）。
- 模型權重 `yolo26m*.pt`、`bucket_name*/` 測試資料（非程式碼，不動）。

## 目標結構

```
video-analyze/     # GPU，重（含 io/、visualization/）
zone-mapping/      # 純 CPU
flow-report/       # 純 CPU，唯一有既有測試
```

每個資料夾自成一體：`pyproject.toml` + `src/<pkg>/` + `tests/` + `README.md` +
`config.toml`。Python 套件名用 snake_case：`video_analyze`／`zone_mapping`／`flow_report`。
...
| `core/registry.py` | `CameraRegistry`/`CameraEntry`/`load_registry`/`resolve_cameras` | 完整（含 `Zone`/`parse_and_validate_zones`） | 完整（含 `load_registry_from_path`） | **複製兩種形狀**：analyze 精簡版；zone-map 與 report 各一份完整版（內容相同） |
...
- [ ] 三包輸出與 golden 基線一致（`tracking_results.parquet`／`zone_counts.parquet`／
      `report.xlsx`），fixture 為 `bucket_name1`。
