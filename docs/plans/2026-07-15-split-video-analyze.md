...
移植方式（另行討論）；新增測試（analyze 目前無測試，維持現狀）；模型權重與
`bucket_name*/` 測試資料。
...
- **移除**：`Zone`、`CameraEntry.parsed_zones()`、`parse_and_validate_zones`、
  `load_registry_from_path`（analyze 皆未使用）。
...
**不需** `openpyxl`（那是 report 專屬）。ruff 設定沿用（`line-length=88`、
...
- [ ] 測試方式與驗收情境明確：對 **`bucket_name1`** 測試資料（5.6G，非 112G 的
      `bucket_name`）跑 analyze，輸出 `tracking_results.parquet` 與父計畫任務 0 的
      golden **逐值一致**。
