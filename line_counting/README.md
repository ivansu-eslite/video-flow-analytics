...
- **`crossing_band_px_1080p` 仍需依場景調，解析度那一項已由換算消除**：band 對「走出畫面」
  ...
  12 個人裡 11 個整個消失。注意換算後 4K 攝影機的實際帶寬是 50 px（該台的人形也大一倍，
  佔肩寬比例反而是所有攝影機中最低的）。
- **有些重複計數 band 解不掉**：`出入口119` 的每人平均被計次數 1.48，但基準值從 10 掃到
  30 幾乎不影響它（84 → 78）——那條線的穿越深度中位數 181 px，死區根本碰不到。它的重複
...
crossing_band_px_1080p = 25  # 跨越去抖帶寬（1080p 基準像素）；0 = 細線純零交越
```
...
| | `crossing_band_px_1080p` | `25` | 跨越去抖帶狀死區寬度，以 1080p（寬 1920）為基準的像素值，`>= 0`；逐攝影機依 `frame_width` 換算成實際像素；`0` = 細線純零交越 |
