...
class LineConfig(BaseModel):
    ...
    def _reject_moved_bucket_minutes(cls, data: Any) -> Any:
        """`bucket_minutes` 已從 `[line]` 移到 `[input]`，要報出新位置。

        `extra="forbid"` 本來就會擋下，但訊息只說不允許額外欄位，看不出該搬到哪個
        區塊；而環境變數形式 `LINE__BUCKET_MINUTES` 過去是本包的合法設定，沿用舊設定
        ...
        if isinstance(data, dict) and "bucket_minutes" in data:
            raise ValueError(
                "[line] 的 bucket_minutes 已移到 [input]：時段粒度是四包共用的輸入"
                "參數，`flow_report` 也要對得上同一個值，故收斂成單一區塊。請把它移到"
                " [input] 底下（環境變數由 LINE__BUCKET_MINUTES 改為 "
