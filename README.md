...
| `outputs/{bucket}/report.xlsx` | flow_report | 跨日累加的 Excel 報表（七個分頁：區域三頁、出入口三頁、活動事件一頁） |
...
| [016](docs/adr/zone_mapping/016-zone-dwell-threshold.md) | `zone_mapping` | `zone_counts.parquet` 新增停留人次 `dwell_events`，記錄判定基礎為何接在生的 `in_zone` 上（與 `unique_visitors` 同源、與 `entries` 同型）、加一道「該段要真的深入區內」的對照組為何被量測結果否決（X=20 s 只差 3.6%，且被排除的段深入 27–42 px）、容忍窗的有效上界為何就是漏偵測空洞的上界 `track_buffer / fps`（實測 2.133 s；預設 3.0 高於它，理由是 T 在 1.5–5.0 秒之間事件數只動 12%、3.0 落在平坦區），以及取樣間隔的 fail-loud 為何必須用 per-track 而非 camera-wide 的統計量（同格多人會把中位數壓成 0，檢查在最需要它的忙碌攝影機上失效） |
| [017](docs/adr/flow_report/017-zone-dwell-sheet.md) | `flow_report` | 報表新增「各區域停留人次」分頁承接 `dwell_events`，記錄為何不放寬 `[report].metric`（三種口徑共用同一個中文表頭「人流量」）、為何不加欄到既有分頁（既有 `report.xlsx` 的表頭永遠不會被更新，6 欄的列會靜默 append 到 5 欄表頭底下）、缺 `dwell_events` 欄的舊 parquet 為何整份 fail loud 而不是跳過該頁（0 與缺資料在報表裡長得一樣）、為何不加停留尖峰頁（母體常是個位數），以及報表為何刻意不標門檻秒數 |
| [018](docs/adr/video_analyze/018-engine-filename-build-identity.md) | `video_analyze` | 引擎產物名改為 `<權重 stem>_sm<SM>_<引擎內容 sha256 前 8 碼>.engine`，記錄 `_sm<SM>` 為何只擋到一半（同權重同卡重建的兩顆引擎檔頭只差一個 `date` 時間戳，附本機三顆 sm75 與兩顆 sm120 引擎的實測 sha256）、為何否決建置時間戳（覆核不了）與建置參數摘要（同參數重建照樣同名）、正式檔名為何要移到驗收通過之後才算（內容 hash 要有檔案才算得出來，暫存名改由 `engine_basename` 直接組），以及載入端為何刻意不驗檔名與內容相符 |
