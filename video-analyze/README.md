...
ultralytics 會**靜默地自動下載** COCO 版權重、不會報錯，屆時 `classes` 過濾會對到錯誤
的類別定義。此權重 best epoch 驗證指標：mAP50≈0.806、recall≈0.72、precision≈0.847。
...
| | `classes` | `[2]` | 要保留的偵測類別 id；權重類別為 `0=head, 1=vbody, 2=fbody` |
